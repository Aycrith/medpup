...
(function () {
    ...
    // --- MOOD DEFINITIONS (v3 — environment tint based) ---
    const MOODS = [
    ...
    function getFragmentShaderSource() {
        return [
            ...
            '    vec3 result = coreColor * core * 2.5;',
            '    result += glowColor * innerGlow * 1.5;',
            '    result += mix(vec3(1.0, 0.1, 0.0), glowColor, 0.5) * outerGlow * 0.4;',
            '    result += glowColor * wideGlow * 0.1;',
            ...
            '    vec3 col = moonSurface * disk * 2.2;',

            // Glow
            '    col += vec3(0.7, 0.85, 1.0) * pow(clamp(moonDot, 0.0, 1.0), 200.0) * 1.2;',
            '    col += vec3(0.7, 0.85, 1.0) * pow(clamp(moonDot, 0.0, 1.0), 70.0) * 0.5;',
            ...
            '    float cloud = smoothstep(0.45, 0.72, c1) * 0.4 + smoothstep(0.5, 0.78, c2) * 0.5;',
            ...
            '    vec3 lit = vec3(1.0, 0.95, 0.9) * 1.3;',
            ...
            'vec3 applyCinematicMood(vec3 col) {',
            '    vec3 tint = getMoodTint();',
            // Subtle color shift toward mood
            '    float gray = dot(col, vec3(0.299, 0.587, 0.114));',
            '    col = mix(col, col * tint, 0.2);',
            '    return col;',
            '}',
            ...
            '        col += vec3(0.08, 0.03, 0.12) * envTint * neb * trueNight * smoothstep(0.0, 0.3, rd.y) * 0.6;',
            '    }',
            ...
            // Water color with wave-local variation
            '            float waveLocalH = GetWaterHeight(p, false);',
            '            vec3 tidalColor = mix(u_waterColor * 0.45, u_waterColor * 2.2, smoothstep(-u_waveHeight, u_waveHeight * 1.5, waveLocalH)) * moodTint;',

            // Night water
            '            vec3 nightWater = mix(envTint * 0.003 + tidalColor * 0.05, envTint * 0.02 + tidalColor * 0.25, max(0.0, moonDir.y));',
            '            vec3 waterBase = mix(nightWater, tidalColor, dayFactor);',
            '            waterBase *= 0.85 + 0.15 * smoothstep(0.0, 20.0, d);',
            ...
            '            float specTerm = pow(clamp(specDot, 0.0, 1.0), specPower) * 4.5 * specDistFade;',
            '            specTerm += pow(clamp(specDot, 0.0, 1.0), 80.0) * 0.8 * specDistFade;',
            ...
            // Color grading
            '    col = applyCinematicMood(col);',

            // Reinhard tone mapping
            '    col = (col * (2.51 * col + vec3(0.03))) / (col * (2.43 * col + vec3(0.59)) + vec3(0.14));',
